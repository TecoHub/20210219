...
print_r($response);               
			...
			$response="12,13,14:333,555,666:vicky,medihi,raghu"; 
  ...
  <div class="containerPlus">
       ...
       <div>
           <form action="menu01.php">
             <select class="select_mate " name="" onchange="update_session_value(this.value)" onclick="return false;" id="">
                <option value=""  >お部屋番号を選択 </option>
                
<?php
for($p=0;$p<sizeof($tablenoarr);$p++){
$val=$tablenoarr[$p]."-".$roomnoarr[$p]."-".$guestnamearr[$p];

echo " <option value=".$val.">".$roomnoarr[$p]."</option>";
}
?>                
				</select>
